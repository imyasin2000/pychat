...
class Socket:
    ...
    def _on_connect(self, client: socket):
        ...
        print(self.user_info["user"], ' joined at ',datetime.now().strftime("%I:%M:%S"))


    def _on_disconnect(self, client: socket):
        if client in self.clients:
            print(self.clients.get(client), 'disconnected at ',datetime.now().strftime("%I:%M:%S"))
    ...
    def _on_message(self, client: socket, data: bytes):
        ...
        for toclient, username in self.clients.items():
            if username == x["toclient"]:
                ...
                print("The message was sent by {0} to {1} at {2}".format(x["username"],x["toclient"],datetime.now().strftime("%I:%M:%S")))
        pass
